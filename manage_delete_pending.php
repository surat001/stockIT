...
if (!empty($document_number)) {
    $sql = "SELECT column_name, status_balance, created_at_update, user_created ,user_operation, created_at, SUM(quantities) as total_quantities 
            FROM delete_pending WHERE document_number = ? LIMIT 1";
...
$(document).ready(function() {
    ...
    let deleteTable = $("#deleteTable").DataTable({
        "columnDefs": [{
                "targets": "_all",
        ...
        "ajax": {
            "url": "includes/fetch_delete_by_document.php",
            "type": "GET",
            "data": {
    ...
    setTimeout(function() {
        deleteTable.columns.adjust().draw();
    }, 500);
});
</script>
...
#deleteTable thead th {
    text-align: center;
    white-space: nowrap;
}

/* ✅ ปรับขนาดให้ตารางตรง */
#deleteTable tbody td {
    text-align: center;
